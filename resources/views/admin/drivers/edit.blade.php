                ...
                <div class="p-6 pt-0">
                    <form method="POST" action="{{ route('admin.drivers.update', $driver) }}" class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input id="name" name="name" type="text" value="{{ old('name', $driver->name) }}" placeholder="Driver full name" required class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                            @error('name')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <input id="phone" name="phone" type="text" value="{{ old('phone', $driver->phone) }}" placeholder="09XX XXX XXXX" class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                                @error('phone')
                                    <p class="text-xs text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="space-y-2">
                                ...
                                <input id="license_number" name="license_number" type="text" value="{{ old('license_number', $driver->license_number) }}" placeholder="Driver's license number" class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
                        ...
                        <div class="space-y-2">
                            ...
                            <select id="status" name="status" class="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
                                ...
                                <option value="dispatched" {{ old('status', $driver->status) === 'dispatched' ? 'selected' : '' }}>Dispatched</option>
                                <option value="on_route" {{ old('status', $driver->status) === 'on_route' ? 'selected' : '' }}>On Route</option>
                        ...
                        <div class="flex items-center gap-2">
                            <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $driver->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border border-primary shadow focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" />
